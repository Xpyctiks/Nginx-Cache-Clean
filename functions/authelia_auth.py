...
def try_authelia_login():
  ...
  Registered as a before_request hook so it runs on every request before the view function."""
  ...
  if not user:
    ...
    flash('Your identity was confirmed via Authelia, but no matching account exists in this system. Contact the administrator.', 'alert alert-danger')
